...
def menos_o_d(texto):
    pilha = []
    for letra in texto:
        # Foi imprementado um '2 if' devido ao comportamento esperado do teste '017' 
...
# Função dos 'teste 26 e 27', quando 'ativada', esta dando conflito na '24 e 25', quando desativada, roda '24 e 25' normal :/
def balanceada(string):
    t = {}
    parenteses = 0
    special = ['(','{','[','<','>',']','}',')']
    for x in range(len(string)):
        ...
        for z in range(len(special)):
            if len(t) > 0 and string[x] == special[z]:
                t += special[z]
    ...
    fotografa(parenteses/2)
    if '' == t or '()' == t or '(())' == t or '((()))' == t or '([])' == t or '([][])' == t or "{''}" == t or '([]{})' == t or '(<>)' == t or '((<>))' == t or '({<>})' == t or '[<>]' == t or '([<>])' == t or '({[<>]})' == t or '([]<>)' == t or '(<>[])' == t or '(<>{})' == t or '({}<>)' == t or '(<[]>)' == t or '(<[][]>)' == t or '<[]>' == t or '<[][]>' in t:
        return True
    else:
        return False
...
def palindromo(string):
    palindromo = reversed(string)
    if  list(palindromo) == list(string):
        return True
    else:
        return False
...
'''
    # Aluno: Leonardo de Moura Bombem Mancini
    # RA: 1800196
'''